...
double measure_time(auto&& func, const std::vector<std::vector<double>>& matrix, const std::vector<double>& vector, int m, int n) {
...
void run_experiment(int m, int n, const std::vector<int>& threads,std::ofstream& stream) {
    std::cout << "\n=== Matrix size: " << m << "x" << n << " ===\n";
    ...
    for (int t : threads) {
        ...
        double sum=0;
        for(int i=0;i<n;++i){
            double measured_time = measure_time(parallel_product, matrix, vector, m, n);
            sum += measured_time;
        }
        stream << t << " thread(s): " << sum/(double)n 
                  << "ms"<<std::endl;
    }
}
...
int main() {
    ...
    run_experiment(20000, 20000, thread_counts,output_stream);

    run_experiment(40000, 40000, thread_counts,output_stream);
    output_stream.close();
    return 0;
}